...
def predict_ber_ook(snr_linear):
    """
    Predict BER for OOK modulation given linear SNR.
    BER = Q(sqrt(SNR)) = 0.5 * erfc(sqrt(SNR/2))
    """
    return 0.5 * erfc(np.sqrt(snr_linear / 2))

def compute_eb_n0(snr_linear, bandwidth_hz, bit_rate_bps):
    """
    Compute Eb/N0 from SNR.
    Eb/N0 = SNR * (B / R)
    """
...
class Demodulator:
    ...
    def __init__(self, params=None):
        ...
        self.hpf_cutoff = params.get('hpf_cutoff', 1e4)  # 100 kHz
        self.lpf_cutoff = params.get('lpf_cutoff', 4e5)  # 2 MHz
...
def test_demodulator():
    ...
    # Validation
    assert result['ber_stats']['ber'] <= 0.1, "[ERROR] BER too high!"
